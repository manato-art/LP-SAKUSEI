...
describe('期間プリセット（採取した select の option value）', () => {
  ...
  // 2026-09-24 点検19: 選ぶとエラーが出ていた。「過去7日間」（今日を含む7日）と別の選択肢なので、
  // 「7日間」は**昨日までのまる7日間**（今日を含まない）とする（本人へ報告済みの決め事）。
  it('「7日間」は昨日までのまる7日間（今日を含まない）', () => {
    expect(resolvePreset('seven_days', today)).toEqual({
