...
- サブタブ: `レポート` / `ヒートマップ`
- ヘッダ右: `広告データ取得日時` / ⚙ / 💡
  - ⚙ = **パラメーター設定**（モーダルを開くトリガー）。採取CSSの `_trigger_wn8sv_1` が
    `settings_blue-2d549b85.svg`（`icon_settings`）を背景に持ち、名札は `パラメーター設定`。
    親の `_trigger_11n4w_113` は `cursor:pointer`（`11n4w` はモーダル用モジュール）。
    **開いた状態は未採取**なので、中身（列見出し・文言・保存後の挙動）は分からない。
  - 💡 = **ダーク⇄ライトの切替**（`_toggleTheme_8ygjt_83`。点灯=`lamp.svg` / 消灯=`lamp-off.svg`）。
  - `広告データ取得日時` は押すと開く小さな面。中身は採取済み
    （`beyondページ 広告データ取得日時` / `※ 配信金額、クリエイティブ画像/テキストを取得しています` / `データなし`）。
  - ※ 以前ここに書いていた「⚙（列選択）/ 🔔（通知？）」は**どちらも誤り**（2026-09-15に採取物で確認して訂正）。
...
| **Widget管理メニューの開いた状態** | `tool-widget-open` を採り直しても採れていない（`_open_1ti69_52` がどの採取物にも1件も無い）。開いた位置・サイズは実CSS（`._actionDropdownBody_1ti69_5` = `position:absolute` / `width:200px` / top・left指定なし）から読むしかない |
| リンクパネルの `ページ内移動` タブの中身 | `toolbar-link-open` では tabpanel が空（Radixが遅延描画）。**外部リンク側だけ採れている** |
| リンクパネルの成果単価の通貨セレクトの選択肢 | `aria-expanded="false"` のままで一覧が出ていない |
| リンク追加時に飛ぶAPI／`<a>` に載る属性 | LPが空でリンクを実際に作っていないため未観測（`href` 以外は不明） |
