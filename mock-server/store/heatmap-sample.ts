/**
 * 計測タグが離脱時に送る「1回の表示ぶんの位置の記録」を読み直し、集計の行へ足す（track-heatmap.ts から分離・2026-09-25）。
 *
 * ふつうのヒートマップ（heatmapStats・deviceHeatmapStats）と、申し込んだ人だけのヒートマップ（cvHeatmapStats）の
 * 両方が同じ足し方を使う。足し方を2か所に書くと必ずずれる。
 */
import type { HeatmapStat } from './types.ts'
...
export const CLICK_CAP = 5000
/** 端末ごとの行に持つクリックの上限（行が増えるぶん、保存の容量を抑える） */
export const DEVICE_CLICK_CAP = 2000

export function readSample(body: unknown): { sample: HeatmapSample; params: string[] } {
  const hb = body as {
    bands?: unknown
    reach?: unknown
    dwell?: unknown
 ...
 */
export function mergeSample<T extends HeatmapStat>(
  stats: readonly T[],
  isSame: (h: T) => boolean,
  fresh: () => T,
...
export function emptyStat(abTestUid: string, versionUid: string, date: string, bands: number, param: string): HeatmapStat {
  return {
