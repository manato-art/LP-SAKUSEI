...
import { collectKnownEntries, mergeMaps, parseKnownEntries, type ScrubMap } from './dictionary.ts'
import { buildCollectedMap } from './collect.ts'
import { loadRouteWordsFromManifest } from '../shared/url-identifier.ts'
import { isJsonFile, isTextFile, scansUrlIdentifiers } from './scan-targets.ts'
...
function main(): void {
  ...
  // 1) 辞書構築（どのファイルから何を拾うかは collect.ts に集約）
  const map: ScrubMap = loadMap()
  const routeWords = loadRouteWordsFromManifest(readFileSync('docs/routes.json', 'utf8'))
  const collected = buildCollectedMap(files, (file) => readFileSync(file, 'utf8'), routeWords)
  console.log(`[scrub] URL形のID走査: ${files.filter(scansUrlIdentifiers).length}ファイル`)
