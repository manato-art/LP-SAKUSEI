 ...
 *   配信幅の body・LP の共通CSS・Version の CSS・記事設定の CSS・Widget の土台・フォント・Widget の外部スタイルシート
 * 入れないもの:
...
export function buildHeatmapLpDocument(source: HeatmapLpSource): string {
  ...
  const lp = neutralizeWidgetStyles(source.html)
  return (
    ...
    `</head><body>${withAutoplayVideos(lp.html)}</body></html>`
  )
}
