 ...
 * 設定画面の「テーマカラー」。
 *
 * 選んだ色はその場で画面全体に反映し、アカウントに保存する
 * （`theme-color.ts` が `:root` の CSS 変数へ流し込む）。
 */
...
export function buildThemeColorSection(): HTMLElement {
  ...
  wrap.append(
    ...
    el('div', {
      text: 'ボタンや選択中の項目に使う色です。選ぶとすぐ画面に反映され、次回以降も同じ色になります。',
  ...
  for (const preset of ACCENT_PRESETS) {
    const dot = el('button', {
      style: [
        `width:34px;height:34px;border-radius:50%;border:1px solid rgba(0,0,0,.12)`,
  ...
  customInput.style.cssText =
    'width:38px;height:34px;padding:0;border:1px solid #DDD;border-radius:6px;background:#fff;cursor:pointer'
