...
function resolvePanel(root: HTMLElement): HTMLElement | null {
  ...
  if (fromSubstrate !== null) {
    // パネルの親 _dropdown_ が sideToolbarWrapper 内に居ると overflow:hidden でクリップされる。
    // root 直下に移して position:fixed にし、クリップチェーンから抜け出す。
    const host = fromSubstrate.closest<HTMLElement>(`.${CLS.dropdown}`)
    if (host !== null && !host.hasAttribute('data-clone-panel-host')) {
